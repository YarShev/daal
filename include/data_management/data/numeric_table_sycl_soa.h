    ...
    services::Status serialImpl(Archive * arch)
    {
        ...
        else
        {
            for (size_t i = 0; i < ncol; i++)
            {
                ...
                TypeDispatcher<SyclEventNoExist>::dispatch(_arrays[i].type(), reinterpreter);
...
private:
    ...
    services::Status getTBlock(size_t idx, size_t nrows, ReadWriteMode rwFlag, BlockDescriptor<T> & block)
    {
        ...
        for (size_t j = 0; j < ncols; j++)
        {
            ...
            TypeDispatcher<SyclEventNoExist>::dispatch(featureUniBuffer.type(), converter);
    ...
    services::Status releaseTBlock(BlockDescriptor<T> & block)
    {
        ...
        if (block.getRWFlag() & (int)writeOnly)
        {
            ...
            for (size_t j = 0; j < ncols; j++)
            {
                ...
                TypeDispatcher<SyclEventNoExist>::dispatch(uniBuffer.type(), converter);
